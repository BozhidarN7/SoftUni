...
public class PersonCollection : IPersonCollection
{
    ...
    {
        string townAndName = person.Town + " " + person.Name;
        if (!this.byNameAndTown.ContainsKey(townAndName))
        {
            this.byNameAndTown[townAndName] = new SortedDictionary<string, Person>();
        }

        this.byNameAndTown[townAndName].Add(person.Email, person);
    }

    private void AddByDomain(string email, Person peron)
    {
        ...
        this.byDomain[domain].Add(email, peron);
    }
    ...
    public bool DeletePerson(string email)
    {
        ...
        string domain = email.Split('@')[1];
        this.byDomain[domain].Remove(person.Email);
