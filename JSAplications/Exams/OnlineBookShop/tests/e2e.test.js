...
describe('E2E tests', function () {
    ...
    describe('Authentication [ 20 Points ]', () => {
        it('register does not work with empty fields [ 5 Points ]', async () => {
            const { post } = await createHandler(endpoints.register, {
                post: {},
            });
        ...
        it('login makes correct API call [ 5 Points ]', async () => {
            const data = mockData.users[0];
            const { post } = await createHandler(endpoints.login, {
                post: data,
            });
    ...
    describe('Navigation bar [ 5 Points ]', () => {
        ...
        it('logged user should see correct navigation [ 2 Points ]', async () => {
            ...
            //Test for navigation
            expect(await page.isVisible('nav >> text=Dashboard')).to.be.true;
            expect(await page.isVisible(`nav >> text=Welcome, ${data.email}`))
                .to.be.true;
    ...
    describe('Catalog [ 20 Points ]', () => {
        it('show empty catalog [ 2.5 Points ]', async () => {
            ...
            expect(await page.isVisible('text=No books in database!')).to.be
                .true;
        });
        ...
        it('show details [ 5 Points ]', async () => {
            ...
            expect(await page.isVisible(`h3:has-text("${data.title}")`)).to.be
                .true;
            expect(await page.isVisible(`.type:has-text("${data.type}")`)).to.be
                .true;
            expect(
    ...
    describe('CRUD [ 45 Points ]', () => {
        ...
        it('create does NOT work with empty fields [ 5 Points ]', async () => {
            const { post } = await createHandler(endpoints.create, {
                post: '',
            });
        ...
        it('delete makes correct API call for logged in user [ 5 Points ]', async () => {
            const data = mockData.catalog[0];
            const { delete: del } = await createHandler(
                endpoints.details(data._id),
                { get: data, delete: '' }
            );
    ...
    describe('BONUS : Like functionality  [ 10 Points ]', async () => {
        it('Like button is not visible for guest users [ 1 Points ]', async () => {
            ...
            expect(await page.isVisible('.actions .button >> text="Like"')).to
                .be.false;
