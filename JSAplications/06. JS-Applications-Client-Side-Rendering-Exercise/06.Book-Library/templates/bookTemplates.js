...
export const allFormsTemplate = (forms) =>
    html`${forms.map((f) => formTemplate(f))}`;
...
>
    ...
    <input
        ...
        name="author"
        placeholder="Author..."
        .value=${form.authorValue}
    />
    <input type="submit" value=${form.submitText} />
</form>`;

export const bookTemplate = (book, editHandler, deleteHandler) => html` <tr
    class="book"
    data-id=${book._id}
>
    <td>${book.title}</td>
...
export const allBooksTemplate = (books, editHandler, deleteHandler) =>
    html`${books.map((b) => bookTemplate(b, editHandler, deleteHandler))}`;
...
export const bookLibaryTemplate = (
    books,
    forms,
    loadBooksHandler,
    editHandler,
